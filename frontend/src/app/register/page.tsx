...
import { Zap, ArrowRight, Mail, Lock, User, Briefcase, Loader2 } from 'lucide-react';
...
export default function Register() {
  const [formData, setFormData] = useState({
    ...
    firstName: '',
    lastName: '',
    role: 'talent' as 'talent' | 'recruiter' | 'admin'
  });
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <h2 className="text-center text-4xl font-black text-slate-900 tracking-tight">
          Join Umurava<span className="text-primary-600">AI</span>
        </h2>
        <p className="mt-2 text-center text-slate-500 font-medium tracking-tight">
          Start your journey with the power of artificial intelligence.
        </p>
      </div>
        ...
        <div className="glass-card py-10 px-8 shadow-2xl border-slate-200/50">
          <form className="space-y-5" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label htmlFor="lastName" className="block text-xs font-black text-slate-500 uppercase tracking-wider ml-1">
                  Last Name
                </label>
                <input
                  id="lastName"
                  ...
                  onChange={handleChange}
                  className="block w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all font-medium"
                />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              <label htmlFor="role" className="block text-xs font-black text-slate-500 uppercase tracking-wider ml-1">
                Account Type
              </label>
              <div className="relative group">
                <Briefcase className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-primary-500 transition-colors" />
                <select
                  id="role"
                  name="role"
                  value={formData.role}
                  onChange={handleChange}
                  className="block w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 transition-all font-bold text-slate-700 appearance-none cursor-pointer"
                >
                  <option value="talent">Talent (Finding Jobs)</option>
